...
class BaseDecodeHeadContrast(BaseModule, metaclass=ABCMeta):
    ...
    def loss_by_feat(self, seg_logits: Tensor,
        ...
        loss = dict()
        seg_logits['decode'] = resize(
            input=seg_logits['decode'],
            size=seg_label.shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        if self.sampler is not None:
            seg_weight = self.sampler.sample(seg_logits['decode'], seg_label)
        else:
            seg_weight = None
        ...
        loss['acc_seg'] = accuracy(
            seg_logits['decode'], seg_label, ignore_index=self.ignore_index)
        return loss
